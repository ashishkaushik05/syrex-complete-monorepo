...
TMP_SCHEMA="$BACKEND_DIR/.tmp.phase3.schema.prisma"
PG_CONTAINER="syrex_phase3_pg"
PG_PORT="54332"
API_PORT="3003"
...
if ! command -v docker >/dev/null 2>&1; then
  echo "docker is required for phase3 smoke test" >&2
  exit 1
fi
...
DATABASE_URL="$DB_URL" bun run scripts/phase3-seed.ts >/tmp/syrex_phase3_seed.log 2>&1

DATABASE_URL="$DB_URL" PORT="$API_PORT" bun run src/index.ts >/tmp/syrex_phase3_api.log 2>&1 &
API_PID=$!
...
PHASE3_BASE_URL="http://127.0.0.1:${API_PORT}" DATABASE_URL="$DB_URL" bun run scripts/phase3-smoke-client.ts
