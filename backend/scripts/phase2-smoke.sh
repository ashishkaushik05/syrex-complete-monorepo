...
TMP_SCHEMA="$BACKEND_DIR/.tmp.phase2.schema.prisma"
PG_CONTAINER="syrex_phase2_pg"
PG_PORT="54331"
API_PORT="3002"
...
if ! command -v docker >/dev/null 2>&1; then
  echo "docker is required for phase2 smoke test" >&2
  exit 1
fi
...
DATABASE_URL="$DB_URL" bun run scripts/phase2-seed.ts >/tmp/syrex_phase2_seed.log 2>&1

DATABASE_URL="$DB_URL" PORT="$API_PORT" bun run src/index.ts >/tmp/syrex_phase2_api.log 2>&1 &
API_PID=$!
...
PHASE2_BASE_URL="http://127.0.0.1:${API_PORT}" DATABASE_URL="$DB_URL" bun run scripts/phase2-smoke-client.ts

echo "phase2 smoke passed"
